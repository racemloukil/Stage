...
import tn.esps.services.UserService;
...
public class UserInfo {
	...
	@PutMapping("/user")
	public ResponseEntity<Response> editUser(@RequestParam("image") MultipartFile file,
			@RequestParam("user") String user) throws JsonParseException, JsonMappingException, Exception {

		UserInformation userIm = new ObjectMapper().registerModule(new JavaTimeModule()).readValue(user,
...
//       user.setPhoto(newFileName);
	...
	public ResponseEntity<?> createEmploye(@RequestPart("user") String user, @RequestParam("image") MultipartFile file1)
